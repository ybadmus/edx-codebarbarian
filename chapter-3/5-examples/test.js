...
describe('Category API', function() {
  ...
  it('can load all categories that have a certain parent', function(done) {
    var categories = [
      { _id: 'Electronics', ancestors: 'Phones' },
      { _id: 'Phones', parent: 'Electronics' },
      { _id: 'Laptops', parent: 'Electronics' },
      { _id: 'Bacon', ancestors: 'Phones' }
    ];

    // Create 4 categories
    Category.create(categories, function(error, categories) {
      var url = URL_ROOT + '/category/ancestors/Phones';
      ...
      superagent.get(url, function(error, res) {
        ...
        assert.equal(result.categories[0]._id, 'Bacon');
        assert.equal(result.categories[1]._id, 'Electronics');
        done();
      });
    });
  });
